...
| Variable | Purpose |
|----------|---------|
| `HF_TOKEN` | HuggingFace token — accept licence at hf.co/mistralai/Mistral-Nemo-Instruct-2407 |
...
```bash
python fine-tuning/mistral-nemo-12b-grpo/reward.py
```
...
python fine-tuning/mistral-nemo-12b-grpo/evaluate.py

# Skip fine-tuned model — base model only (4 configs)
python fine-tuning/mistral-nemo-12b-grpo/evaluate.py --checkpoint none

# Specific mid-training checkpoint
python fine-tuning/mistral-nemo-12b-grpo/evaluate.py \
    --checkpoint fine-tuning/mistral-nemo-12b-grpo/checkpoints/checkpoint-50

# Dry run — generation only, no G-Eval cost
python fine-tuning/mistral-nemo-12b-grpo/evaluate.py --no-geval
```

Saves to `fine-tuning/mistral-nemo-12b-grpo/results/`:
...
| `vllm_gpu_memory_utilization` | 0.35 | ~28 GB on H100 79 GB |
| `vllm_cache_dir` | `fine-tuning/mistral-nemo-12b-grpo/cache/vllm` | Writable cache for HPC systems |
...
| Component | Est. VRAM |
|-----------|-----------|
| vLLM — Mistral Nemo 12B BF16 (colocate, KV cache at 0.35 util) | ~28 GB |
| Training model — Mistral Nemo 12B BF16 (full precision) | ~24 GB |
